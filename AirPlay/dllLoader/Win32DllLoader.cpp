...
bool Win32DllLoader::Load()
{
  ...
  std::wstring strDllW;
  StrConversion::ANSI_To_UNICODE_String(strFileName.c_str(),strDllW);
  ...
  if (!m_dllHandle)
  {
    ...
    if (strLen != 0)
    {
	  ...
	  StrConversion::UNICODE_To_ANSI_String(std::wstring(lpMsgBuf, strLen).c_str(), strMessage);
      SPLOGAN(LOG_ERROR, "%s: Failed to load \"%s\" with error %lu: \"%s\"", __FUNCTION__, strFileName.c_str(), dw, strMessage.c_str());
    }
    else
      SPLOGAN(LOG_ERROR, "%s: Failed to load \"%s\" with error %lu", __FUNCTION__, strFileName.c_str(), dw);
...
void Win32DllLoader::Unload()
{
  ...
  if (m_dllHandle)
  {
    if (!FreeLibrary(m_dllHandle))
       SPLOGAN(LOG_ERROR, "%s Unable to unload %s", __FUNCTION__, GetName());
  }
...
int Win32DllLoader::ResolveExport(const char* symbol, void** f, bool logging)
{
  ...
  {
    if (logging)
      SPLOGAN(LOG_WARNING, "%s - Unable to resolve: %s %s, reason: DLL not loaded", __FUNCTION__, GetName(), symbol);
    return 0;
  }
  ...
  {
    if (logging)
      SPLOGAN(LOG_WARNING, "%s - Unable to resolve: %s %s", __FUNCTION__, GetName(), symbol);
    return 0;
  }
