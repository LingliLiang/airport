...
CNetwork::CNetwork()
{
	...
	SPLOGA(LOG_INFO,"TMSG_NETWORKMESSAGE - SERVICES_UP");
}

CNetwork::~CNetwork()
{
	//CApplicationMessenger::GetInstance().PostMsg(TMSG_NETWORKMESSAGE, SERVICES_DOWN, 0);
	SPLOGA(LOG_INFO,"TMSG_NETWORKMESSAGE - SERVICES_DOWN");
}
...
int CNetwork::ParseHex(char *str, unsigned char *addr)
{
	...
	while (*str)
	{
		...
		if (str[1] == 0)
			return -1;
		if (sscanf_s(str, "%02x", (unsigned int *)&tmp) != 1)
			return -1;
		addr[len] = tmp;
		len++;
		str += 2;
...
#ifdef TARGET_WINDOWS
	std::string hostStr;
	StrConversion::ANSI_To_UTF8_String(hostName, hostStr);
	hostname = hostStr;
#else
	hostname = hostName;
#endif
...
bool CNetwork::IsLocalHost(const std::string& hostname)
{
	...
	if (StrConversion::StartsWith(hostname, "127.")
		|| (hostname == "::1")
		|| StrConversion::EqualsNoCase(hostname, "localhost"))
		return true;

	std::string myhostname;
	if (GetHostName(myhostname)
		&& StrConversion::EqualsNoCase(hostname, myhostname))
		return true;
...
void CNetwork::NetworkMessage(EMESSAGE message, int param)
{
	...
	{
	case SERVICES_UP:
		SPLOGAN(LOG_DEBUG, "%s - Starting network services",__FUNCTION__);
		CNetworkServices::GetInstance().Start();
		break;

	case SERVICES_DOWN:
		SPLOGAN(LOG_DEBUG, "%s - Signaling network services to stop",__FUNCTION__);
		CNetworkServices::GetInstance().Stop(false); // tell network services to stop, but don't wait for them yet
		SPLOGAN(LOG_DEBUG, "%s - Waiting for network services to stop",__FUNCTION__);
...
bool CNetwork::WakeOnLan(const char* mac)
{
	...
	{
		SPLOGAN(LOG_ERROR, "%s - Invalid hardware address specified (%s)", __FUNCTION__, mac);
		return false;
	}
	...
	if ((packet = socket (AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0)
	{
		...
		SPLOGAN(LOG_ERROR, "%s - Unable to create socket (%s)", __FUNCTION__, err);
		return false;
	}
	...
	if (setsockopt (packet, SOL_SOCKET, SO_BROADCAST, (char*) &value, sizeof( unsigned int ) ) == SOCKET_ERROR)
	{
		char err[1024]={0};
		strerror_s (err,errno);
		SPLOGAN(LOG_ERROR, "%s - Unable to set socket options (%s)", __FUNCTION__, err);
		closesocket(packet);
		return false;
	}
	...
	if (sendto (packet, (char *)buf, 102, 0, (struct sockaddr *)&saddr, sizeof (saddr)) < 0)
	{
		char err[1024]={0};
		strerror_s (err,errno);
		SPLOGAN(LOG_ERROR, "%s - Unable to send magic packet (%s)", __FUNCTION__, err );
		closesocket(packet);
		return false;
	}

	closesocket(packet);
	SPLOGAN(LOG_INFO, "%s - Magic packet send to '%s'", __FUNCTION__, mac);
	return true;
}
...
bool CNetwork::PingHost(unsigned long ipaddr, unsigned short port, unsigned int timeOutMs, bool readability_check)
{
	...
	if (err_msg && *err_msg)
	{
		...
		SPLOGAN(LOG_ERROR, "%s(%s:%d) - %s (%s)", __FUNCTION__, inet_ntoa(addr.sin_addr), port, err_msg, sock_err.c_str());
	}
...
SOCKET CreateTCPServerSocket(const int port, const bool bindLocal, const int backlog, const char *callerName)
{
	...
	if ((sock = socket(PF_INET6, SOCK_STREAM, IPPROTO_TCP)) >= 0)
	{
		...
		if (setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, (const char*)&no, sizeof(no)) < 0)
		{
			...
			std::string sock_err = strerror(errno);
#endif
			SPLOGAN(LOG_WARNING, "%s Server: Only IPv6 supported (%s)", callerName, sock_err.c_str());
		}
		...
		if (bind( sock, (struct sockaddr *) &addr, sizeof(struct sockaddr_in6)) < 0)
		{
			closesocket(sock);
			sock = -1;
			SPLOGAN(LOG_DEBUG, "%s Server: Failed to bind ipv6 serversocket, trying ipv4", callerName);
		}
	}
	...
	if (sock < 0 && (sock = socket(PF_INET, SOCK_STREAM, 0)) >= 0)
	{
		...
		{
			closesocket(sock);
			SPLOGAN(LOG_ERROR, "%s Server: Failed to bind ipv4 serversocket", callerName);
			return INVALID_SOCKET;
		}
	}
	else if (sock < 0)
	{
		SPLOGAN(LOG_ERROR, "%s Server: Failed to create serversocket", callerName);
		return INVALID_SOCKET;
	}
	...
	{
		closesocket(sock);
		SPLOGAN(LOG_ERROR, "%s Server: Failed to set listen", callerName);
		return INVALID_SOCKET;
	}
