...
static inline size_t SizeOfUtf8Char(const char* const str)
{
	if (!str)
		return 0;
...
bool StrConversion::UTF8_To_ANSI_String(const char* pszSrc, std::string &strDes)
{
	strDes.clear();
	_ATLTRY
	{
...
bool StrConversion::UTF8_To_UNICODE_String(const char* pszSrc, std::wstring &strDes)
{
	strDes.clear();
	_ATLTRY
	{
...
bool StrConversion::ANSI_To_UNICODE_String(const char* pszSrc, std::wstring &strDes)
{
	strDes.clear();
	_ATLTRY
	{
...
bool StrConversion::ANSI_To_UTF8_String(const char* pszSrc, std::string &strDes)
{
	strDes.clear();
	_ATLTRY
	{
...
bool StrConversion::UNICODE_To_ANSI_String(const wchar_t* pszSrc, std::string &strDes)
{
	strDes.clear();
	_ATLTRY
	{
...
bool StrConversion::UNICODE_To_UTF8_String(const wchar_t* pszSrc, std::string &strDes)
{
	strDes.clear();
	_ATLTRY
	{
...
void StrConversion::UTF8_To_CString(const char* input, CString& output)
{
	std::string str;
	std::wstring wstr;
	if (sizeof(TCHAR) == 1) {
...
void StrConversion::CString_To_UTF8(CString input, std::string &strDes)
{
	if (sizeof(TCHAR) == 1) {
...
void StrConversion::ANSI_To_CString(const char* input, CString& output)
{
	std::string str;
	std::wstring wstr;
	if (sizeof(TCHAR) == 1) {
...
void StrConversion::CString_To_ANSI(CString input, std::string &strDes)
{
	if (sizeof(TCHAR) == 1) {
...
void StrConversion::Unknown_To_UTF8(std::string &strDes)
{
	std::string stringSrc(strDes);
	if (checkStrForUtf8(stringSrc)!= hiAscii)
	{
		strDes = stringSrc;
	}
	else {
		ANSI_To_UTF8_String(stringSrc.c_str(), strDes);
	}
}

std::string StrConversion::FormatV(const char *fmt, va_list args)
{
	if (!fmt || !fmt[0])
		return "";
	...
	while (1) 
	{
		...
		int nActual = vsnprintf_s(cstr, size,size, fmt, argCopy);
		va_end(argCopy);
...
std::wstring StrConversion::FormatV(const wchar_t *fmt, va_list args)
{
	if (!fmt || !fmt[0])
		return L"";
	...
	while (1)
	{
		...
		va_copy(argCopy, args);
		int nActual = vswprintf_s(cstr, size, fmt, argCopy);
		va_end(argCopy);
...
std::string StrConversion::StrFormat(const char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
...
std::wstring StrConversion::StrFormat(const wchar_t *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
...
void StrConversion::ToUpper(std::string &str)
{
	std::transform(str.begin(), str.end(), str.begin(), toupper);
}

void StrConversion::ToUpper(std::wstring &str)
{
	std::transform(str.begin(), str.end(), str.begin(), toupperUnicode);
}

void StrConversion::ToLower(std::string &str)
{
	std::transform(str.begin(), str.end(), str.begin(), tolower);
}

void StrConversion::ToLower(std::wstring &str)
{
	std:: transform(str.begin(), str.end(), str.begin(), tolowerUnicode);
}

std::vector<std::string> StrConversion::Split(const std::string& input, const std::string& delimiter, unsigned int iMaxStrings /* = 0 */)
{
...
std::vector<std::string> StrConversion::Split(const std::string& input, const char delimiter, size_t iMaxStrings /*= 0*/)
{
	std::vector<std::string> results;
	if (input.empty())
		return results;
...
std::vector<std::string> StrConversion::Split(const std::string& input, const std::vector<std::string> &delimiters)
{
	std::vector<std::string> results;
	if (input.empty())
		return results;
	...
	for (size_t di = 1; di < delimiters.size(); di++)
		StrConversion::Replace(str, delimiters[di], delimiters[0]);
	results = Split(str, delimiters[0]);

	return results;
}

int StrConversion::Replace(std::string &str, char oldChar, char newChar)
{
	int replacedChars = 0;
...
int StrConversion::Replace(std::wstring &str, wchar_t oldChar, wchar_t newChar)
{
	int replacedChars = 0;
...
int StrConversion::Replace(std::string &str, const std::string &oldStr, const std::string &newStr)
{
	if (oldStr.empty())
		return 0;
...
int StrConversion::Replace(std::wstring &str, const std::wstring &oldStr, const std::wstring &newStr)
{
	if (oldStr.empty())
		return 0;
...
bool StrConversion::EqualsNoCase(const std::string &str1, const std::string &str2)
{
...
bool StrConversion::EqualsNoCase(const std::string &str1, const char *s2)
{
	return EqualsNoCase(str1.c_str(), s2);
}

bool StrConversion::EqualsNoCase(const char *s1, const char *s2)
{
...
int StrConversion::CompareNoCase(const std::string &str1, const std::string &str2)
{
	return CompareNoCase(str1.c_str(), str2.c_str());
}

int StrConversion::CompareNoCase(const char *s1, const char *s2)
{
	char c2; // we need only one char outside the loop
	do
	{
...
bool StrConversion::StartsWith(const std::string &str1, const std::string &str2)
{
	return str1.compare(0, str2.size(), str2) == 0;
}

bool StrConversion::StartsWith(const std::string &str1, const char *s2)
{
	return StartsWith(str1.c_str(), s2);
}

bool StrConversion::StartsWith(const char *s1, const char *s2)
{
	while (*s2 != '\0')
	{
		if (*s1 != *s2)
...
bool StrConversion::StartsWithNoCase(const std::string &str1, const std::string &str2)
{
	return StartsWithNoCase(str1.c_str(), str2.c_str());
}

bool StrConversion::StartsWithNoCase(const std::string &str1, const char *s2)
{
	return StartsWithNoCase(str1.c_str(), s2);
}

bool StrConversion::StartsWithNoCase(const char *s1, const char *s2)
{
	while (*s2 != '\0')
	{
...
bool StrConversion::EndsWith(const std::string &str1, const std::string &str2)
{
	if (str1.size() < str2.size())
		return false;
...
bool StrConversion::EndsWith(const std::string &str1, const char *s2)
{
...
bool StrConversion::EndsWithNoCase(const std::string &str1, const std::string &str2)
{
	if (str1.size() < str2.size())
		return false;
...
bool StrConversion::EndsWithNoCase(const std::string &str1, const char *s2)
{
	size_t len2 = strlen(s2);
	if (str1.size() < len2)
		return false;
...
std::string StrConversion::Left(const std::string &str, size_t count)
{
  count = std::max((size_t)0, std::min(count, str.size()));
  return str.substr(0, count);
}

std::string StrConversion::Mid(const std::string &str, size_t first, size_t count /* = string::npos */)
{
...
std::string StrConversion::Right(const std::string &str, size_t count)
{
  count = std::max((size_t)0, std::min(count, str.size()));
  return str.substr(str.size() - count);
}

std::string& StrConversion::Trim(std::string &str)
{
  TrimLeft(str);
  return TrimRight(str);
}

std::string& StrConversion::Trim(std::string &str, const char* const chars)
{
  TrimLeft(str, chars);
...
std::string& StrConversion::TrimLeft(std::string &str)
{
  str.erase(str.begin(), std::find_if(str.begin(), str.end(), std::not1(std::ptr_fun(isspace_c))));
  return str;
}

std::string& StrConversion::TrimLeft(std::string &str, const char* const chars)
{
...
std::string& StrConversion::TrimRight(std::string &str)
{
  str.erase(std::find_if(str.rbegin(), str.rend(), std::not1(std::ptr_fun(isspace_c))).base(), str.end());
  return str;
}

std::string& StrConversion::TrimRight(std::string &str, const char* const chars)
{
...
int StrConversion::asciidigitvalue(char chr)
{
  if (!isasciidigit(chr))
    return -1;
...
int StrConversion::asciixdigitvalue(char chr)
{
