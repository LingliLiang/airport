...
void CThread::SpawnThread(unsigned stacksize)
{
	...
	{
		SPLOGAN(LOG_ERROR, "%s - fatal error %d creating thread", __FUNCTION__, GetLastError());
		return;
	}
	m_ThreadId = threadId;

	if (ResumeThread(m_ThreadOpaque.handle) == -1)
		SPLOGAN(LOG_ERROR, "%s - fatal error %d resuming thread", __FUNCTION__, GetLastError());
...
#pragma pack(pop)
	...
	SetThreadLocalLocale(true); // avoid crashing with setlocale(), see https://connect.microsoft.com/VisualStudio/feedback/details/794122
}
